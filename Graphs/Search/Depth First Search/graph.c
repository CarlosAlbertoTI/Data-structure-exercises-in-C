...
void buscaProfundidade(Grafo *grafo, int inicio, int *vetor_visitados, int cont){
    int i;
    vetor_visitados[inicio] = cont;
    for(i=0; i<grafo->grau[inicio]; i++){
        if(!vetor_visitados[grafo->arestas[inicio][i]])
            buscaProfundidade(grafo,grafo->arestas[inicio][i],vetor_visitados,cont+1);
    }
}
void buscaProfundidade_Grafo(Grafo *grafo, int inicio, int *vetor_visitados){
    ...
    for(i=0; i<grafo->num_vertices; i++)
        vetor_visitados[i] = 0;
    buscaProfundidade(grafo,inicio,vetor_visitados,cont);
