...
void algKruscal_grafo(Grafo *grafo,  int origem,  int * pai){
    int i, j, dest, primeiro, NV = grafo->nro_vertices;
    double menorPeso;
    int *arvore =  (int*) malloc(NV *sizeof(int));
    for (int i = 0; i < NV; i++)
    {
        arvore[i] = i;
        pai[i] = -1;
    ...
    while(1){
        ...
        for (int i = 0; i < NV; i++)
        {
            for (int j = 0; j < grafo->grau[i]; j++)
            {
                if(arvore[i] != arvore[grafo->arestas[i][j]]){
                    if(primeiro){
                        menorPeso = grafo->pesos[i][j];
                        origem = i;
                        dest = grafo->arestas[i][j];
                        primeiro = 0;
                    }else{
                        if(menorPeso > grafo->pesos[i][j]){
                            menorPeso = grafo->pesos[i][j];
                            origem = i;
                            dest = grafo->arestas[i][j];
                        }
        ...
        if(primeiro == 1) break;
        if(pai[origem] == -1) pai[origem] = dest;
        else pai[dest] = origem;

        for (int i = 0; i < NV; i++)
        {
            if(arvore[i] == arvore[dest])
